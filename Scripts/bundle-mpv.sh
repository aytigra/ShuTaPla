...
#
# Run as a build phase after the app binary is linked. It copies libmpv, every Homebrew
# dylib it transitively depends on (ffmpeg, libplacebo, the Vulkan loader, …), and MoltenVK
# into Contents/Frameworks; rewrites every install name to @rpath so the app loads them from
# inside the bundle instead of /opt/homebrew; writes the MoltenVK Vulkan ICD manifest; and
# re-signs each dylib with the app's signing identity so it passes library validation under
...
MOLTENVK_LIB="$BREW_PREFIX/opt/molten-vk/lib/libMoltenVK.dylib"

FRAMEWORKS="$BUILT_PRODUCTS_DIR/$FRAMEWORKS_FOLDER_PATH"
SIGN_IDENTITY="${EXPANDED_CODE_SIGN_IDENTITY:--}"

if [ ! -f "$MPV_LIB" ]; then
    echo "error: libmpv not found at $MPV_LIB — run 'brew install mpv molten-vk'" >&2
    exit 1
fi

mkdir -p "$FRAMEWORKS"
rm -f "$FRAMEWORKS/MoltenVK_icd.json"   # guard against a manifest left here by older builds
...
# MoltenVK is a Vulkan ICD loaded at runtime, not a link-time dependency, so bundle it directly.
if [ -f "$MOLTENVK_LIB" ]; then
    bundle_dylib "$MOLTENVK_LIB"
else
    echo "warning: MoltenVK not found at $MOLTENVK_LIB — Vulkan rendering will be unavailable" >&2
fi

# ICD manifest lives in Resources/ (codesign rejects non-code files under Frameworks/ when it
# seals the bundle). library_path is relative to the manifest → up to Frameworks/.
RESOURCES="$BUILT_PRODUCTS_DIR/$UNLOCALIZED_RESOURCES_FOLDER_PATH"
mkdir -p "$RESOURCES"
cat > "$RESOURCES/MoltenVK_icd.json" <<'JSON'
{
