...
extension AppState {
    ...
    func setManaged(_ playlist: Playlist) {
        exitReviewModes()   // review modes are scoped to one managed playlist; a switch exits them
        remember(playlist)
    ...
    func switchScope(to scope: MediaType) {
        ...
        exitReviewModes()   // review modes are scoped to one managed playlist; a scope switch exits them
        managerScope = scope
