...
final class CloudFileService {
    ...
    /// the matching model and leaves the rest untouched. The feed re-reports unchanged files every
    /// tick, but a restated status invalidates nothing: `cloudStatus` is `@Transient` (so a write
    /// never dirties the context) behind a setter that gates on change (see `PlaylistFile`).
