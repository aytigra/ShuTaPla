...
**Status: complete (code landed; builds clean, unit tests green across the coordinator, app-state, router, and overlay suites).** The audio player UI plus two follow-on redesigns folded in: the **Audio Manager Redesign** (audio as first-class Manager content) and the **Player Library Surface** (one shared three-column overlay body). The audio overlay mounts in `RootView` **only in Player mode**; in Manager, audio is the pinned sidebar transport inlet.

**Manager is a two-scope library.** `AppState.managerScope` (`.visual` = video + image, `.audio`; transient, never persisted — Manager always opens visual) routes the shared sidebar / center / filter / tag machinery to the active scope through accessors (`managerPlaylist`/`managerFiles`/`managerSelection`/`managerFilterMode`/`managerScrollToken`) over parallel slots that never cross-write (`audioSelectedFileIDs` parallels `selectedFileIDs`; `activeServiceFilter` is scope-local and dropped on a scope switch). Audio is list-only (no gallery). A shared `AudioTransport` (state-dependent button set: Stopped → Play·Volume; Playing/Paused add Prev·Stop·Next·Loop) is pinned at the sidebar top in both scopes via `AudioInlet`, with the track name and a click/drag seek bar once a current track exists. In Manager, selecting a different audio playlist **stops the live one** and leaves the new one active + Stopped (`selectAudioInManager`); the inlet's Play cascades none → add-flow, exist → first, active → continue. The Manager shell is an AppKit `NSSplitViewController` (`ManagerSplitScene`) with a native toolbar — scope tabs (`ScopeTabButton`, click-active-to-collapse) + New Playlist `+`, `.navigationTitle`, and scope-specific detail actions — coordinated through `@Observable ManagerChrome`; the `+` overflows on sidebar collapse rather than relocating (accepted limitation, since SwiftUI owns the window content). `DurationService` serves any timeline-bearing file (video + audio).
