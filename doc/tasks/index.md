...
- All enums: `MediaType`, `ImageFitMode`, `ViewMode`, `FilterMode`, `TaggingStatus`, `PlaybackState`, the `Codable` `ServiceFilter` (persisted on `FilterState`), plus runtime-only `CloudStatus`
...
- `PlaylistSidebar.swift` — sections by media type (Video, Image), playlist rows with selection, Audio scope with full management and the pinned `AudioInlet` transport at the top
...
**Status: complete.** `TagSidebar` (right panel: `FilterBar` over the selected playlist's tags + `TagEditorView` for the file-list selection), `FilterBar` (AND/OR tag cloud, saved-search recents, service-filter banner, per-tag playlist-wide rename/remove via context menu), `TagEditorView` (common-tag chips, autocomplete input with enter/arrow/esc handling, invalid-tagging files get a plain rename field and are excluded from batch ops), and `FlowLayout` (wrapping chip layout). `AppState` gains the filtering and tag-editing API, each filter method targeting a given playlist's persisted `filterState` (`toggleFilterTag`/`setFilterMode`/`clearTagFilter`, `toggleServiceFilter` mutually exclusive and overriding the tag filter), `addTag`/`removeTag` (batch, invalid files excluded), `renameTagAcrossPlaylist`/`removeTagAcrossPlaylist`, and saved searches (`saveCurrentSearch`/`applySavedSearch`/`removeSavedSearch`, 10 recent unique, move-to-top). The file list derives from the playlist (`displaySequence`) rather than a cached array, so `FileListView` re-derives on its own when the model changes. Center-panel counter notices toggle the service filter. The whole filter — tag filter and service filter alike — is persisted per playlist. 22 `AppStateTests` passing.
...
- Service filters (Untagged / Invalid tagging / Skipped): activated/deactivated by clicking the center's counter notices, mutually exclusive with each other, temporarily override the tag filter while active; the toggles live in Manager, but the filter itself is honored everywhere (overlays, playback)
...
- Filtered file list: derived from the playlist (`displaySequence`), not a cached array; views re-derive on model change
- The whole filter is persisted per playlist — tag filter and service filter alike
...
As `PlaybackSource`, the coordinator resolves the next/previous file from the current file's owning playlist `playbackSequence` (the playlist's effective filter applied, skipped files dropped, in `sortOrder`), wrapping past the last to the first and before the first to the last; `url(for:)` resolves through the folder's reference-counted scoped-access session, opened when a playlist starts and released when it stops. Playback order follows the active filter and is never reshuffled. `Playlist.playbackSequence` lives on the model — it is `displaySequence` (the effective tag-or-service filter) minus skipped files — so every surface and the coordinator share one rule; the wrap-around index math is an `Array.cyclicSuccessor/cyclicPredecessor` extension reused by the test source double.
...
**Player overlay is one `AudioOverlay` with a compact and an expanded state.** The compact transport bar is always present; the chevron expands the shared `LibrarySurface` (`Views/Shared/`) below it. `LibrarySurface` is the reusable `selector | files | tags` body — a single-`MediaType` playlist selector with a `+`, the filtered file list with scroll-to-current, and a tag editor for the current file — driven by a `LibraryContext` of channel-specific slots/actions. The visual `FilesTagsOverlayView` wraps the same `LibrarySurface`, so audio and visual share one body; the standalone left-hover `PlaylistsOverlay` and the `.playlistsSidebar` overlay state are gone, and player-mode playlist switching lives in the overlay selectors (one media type each, no video↔image jump). `OverlayManager` distinguishes a hover-revealed compact overlay (auto-closes on cursor leave) from a hotkey-revealed one (`audioCompactPinned`, stays until explicitly closed), grants key context only once the slide-in completes (`audioDidFullyReveal()` fired from a `RootView` reveal task; a graze that leaves first cancels it), and `collapseAudioToCompact()` inverts `expandAudioToExtended()`. The top-edge hover (a growing `HoverZone` so moving onto the bar doesn't dismiss it) opens compact; `ControlButtonStyle` is shared, and the add-playlist flow is centralized on `RootView`.
