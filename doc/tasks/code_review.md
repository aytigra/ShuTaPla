...
- [x] `ShuTaPla/State/AppState.swift` — **FIXED** (the substantive part; the rest is by-design). The "differ only in which reconcile they call / two state machines in lockstep" complaint was already resolved by C2/C3, which moved channel reconciliation *into* `deleteFiles` — no confirm flow reconciles per-channel anymore. The residual triplication (the `runConfirmation { deleteFiles + set error }` body) is folded into one `performDelete(_:onError:)` that all three confirms (Manager/Player/audio) call. The candidate/error *fields* stay separate deliberately: the audio and player delete alerts are mounted in different views that can be on screen at once (the 234–237 comment), so merging them would regress the design. Covered by the existing `confirmPlayerDeleteSurfacesFailureAndKeepsFile` / `confirmAudioDeleteAdvancesPastTheTrashedTrack` / `confirmManagerDeleteAdvancesTheLiveAudioChannel` (green before and after).

### D2 — `reconcileAudioSelection` clones `reconcileVisualSelection` (HIGH cost)
- [x] `ShuTaPla/Engines/PlaybackCoordinator.swift` — **FIXED.** Folded both into one `reconcile(playlistThatChanged:)` that reuses the existing `channel(of:)` classification (its `nil` return *is* the "not live on a channel" guard, replacing the per-method `guard let visualPlaylist`/`audioPlaylist`). The shared guard/`playbackSequence` walk/jump-to-first is written once; only the empty-case settle branches by channel (visual image/video engine stop vs `stopAudio()`), keeping the C6 asymmetry. The AppState duplication collapses too: `reconcileChannels(for:)`, `filterChanged`, and the inline pair all become one `coordinator.reconcile(playlistThatChanged:)` call (which also lifts the `coordinator.visualPlaylist === playlist` reach-throughs out of AppState). Six reconcile tests updated to the real entry point, green before and after.

### D3 — `selectVisualPlaylistInPlayer` and `selectAudioPlaylist` duplicate the play-on-select pattern (MEDIUM)
- [x] `ShuTaPla/State/AppState.swift` — **FIXED** (the shared tail; the rest is a deliberate difference). The duplicated "respawn the re-scan" tail (`updateTask?.cancel()` + `updateTask = Task { update }`) in both `select` and `selectAudioPlaylist` is now one `rescan(_:)` helper (shared with D5). The remaining `isNewSelection → coordinator.play` lines stay per-method because the two operate on deliberately independent slots — `select` drives the managed slot (with `managerSelection` seeding and audio-channel release), `selectAudioPlaylist` the audio-channel slot (independent of the Manager, per the 599–603 comment) — so forcing one parameterized method would just re-expand that difference inside a channel switch. Covered by `reselectingTheSamePlaylistReScansTheFolder` and the audio/visual select tests.

### D4 — `AudioInlet` hand-rolls a drag-seek bar that `AudioOverlay` expresses with a `Slider` (MEDIUM)
- [x] `ShuTaPla/Engines/PlaybackCoordinator.swift` + the two views — **FIXED** (shared logic, not a shared view). The two widgets are visually distinct by design — a 4 pt capsule in the sidebar vs a time-labelled `Slider` in the overlay — so one parameterized view would be the wrong altitude. The genuinely duplicated part (the fraction↔seconds mapping, the `duration <= 0` guard, the clamping) is centralized on the coordinator: `audioProgressFraction` (0…1, clamped), `seekAudio(toFraction:)` (the one fraction→seconds map, targeting the live `audioPlaylist`), and `audioIsSeekable`. Both `AudioInlet.seekBar` and `AudioOverlay.scrubber` now read those instead of hand-rolling the math, each keeping its own look. No unit seam exists for the arithmetic (the engine's `currentTime`/`duration` are `private(set)`, fed by libmpv), so it's covered by the engine-level seek tests (`seekMovesTimePosition`/`seekMovesTime`) plus a clean build; the 93-test coordinator/AppState slice stays green.

### D5 — `select()` and `selectAudioPlaylist` share one `updateTask` field → cross-channel re-scan cancellation (LOW)
- [x] `ShuTaPla/State/AppState.swift` — **CONFIRMED & FIXED.** Reproduced red first: `selectingADifferentPlaylistDoesNotCancelTheFirstScan` selects playlist A then a *different* playlist B and asserts A's scan wasn't cancelled — on the single-`updateTask` code A's task came back `isCancelled == true`. Re-scans are now keyed per playlist in `updateTasks[playlist.id]` (via the shared `rescan(_:)` helper), so re-reading one playlist supersedes only *its own* stale scan; `updateTask` remains the most-recent pointer for `delete` to cancel and for tests to await. `delete` cancels just the deleted playlist's entry; the Manager `[esc]` (`cancelInProgressOperation`) cancels all. Test green after the fix; same-playlist supersede (`reselectingTheSamePlaylistReScansTheFolder`) still holds.
