...
delta. *(Deferred: live cloud-status refresh is Task 17 — Cloud / offline file handling, which is
unstarted — so this is out of scope per the review's own scope note. The initial scan still sets
status correctly.)*
...
possible. *(Won't-fix as stated: `savedSearches` is stored as a single Codable attribute, so any
mutation — in place or wholesale — re-encodes the entire blob; avoiding that needs a separate
