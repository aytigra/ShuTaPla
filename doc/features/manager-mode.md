...
- **Center** — the [Filter Strip](filtering.md) over the file list it narrows. The strip carries the tag filter and its saved searches, and the counters for untagged / invalid tagging / skipped files (each activating its Service Filter or, for skipped, its review — see [Service Filters](filtering.md#service-filters)). Serves the Managed Playlist.
- **Right collapsible panel** — Tag management, shown in one of two modes selected from a toolbar button next to the panel's show/hide control. Filtering is not here; it lives in the center strip, over the list it narrows. The default mode shows a file(s) Tag Editor for the currently selected file(s) in the center (the same multi-select tag input described under [Tag editing UI](tags.md#tag-editing-ui)); its heading reflects the selection — **File Tags** for a single file, **Common Tags** for a multi-selection (add/removes tags shared by ALL selected files). The toolbar button switches the panel into **Manage Tags** mode (see [Playlist-wide tag operations](#playlist-wide-tag-operations)) and back; entering it reveals the panel if it was hidden. It edits the Managed Playlist.
...
## Opening at the current file

Whenever a file list appears — selecting a playlist, switching Scope, returning from the player, or launching the app — it opens already positioned at the **current file** (the playlist's last-played file, highlighted as the selection), with no visible scroll travel even when that file sits far down the list. Re-selecting the already-active playlist recenters on it instantly, and a filter or Scope change that reflows the list re-opens positioned the same way. Moving the selection with the arrow keys scrolls it into view with a short animation. When the current file is filtered out, or nothing has played yet, the list opens at the top instead. The gallery and the Player-mode overlay file lists position on the current file identically.
...
- **`[space]` (preview)** — with **exactly one** file selected in a video or image playlist, opens a controls-free floating card, centered over a dimmed backdrop below the toolbar, that peeks at just that file — leaving the playlist, the live audio channel, and the window mode untouched (the preview runs on its own playback engine). The card is sized to the media's aspect ratio: a video plays from the beginning, loops forever at the playlist's volume, and carries a thin progress strip along its bottom edge; an image is shown static (no pan/zoom). The looping video can be seeked without any on-screen transport: `[arrow left]`/`[arrow right]` step ∓3 s (matching the main player's `[right option]+arrow`), and a click or drag anywhere along the bottom strip — a taller invisible hit zone over the thin fill — seeks to that fraction of the duration. Seeking is video-only; the strip is hidden and the arrows do nothing for an image. The card appears at its true shape once the size is known: immediately for an image, and for a video too once its dimensions have been cached from an earlier display — only a video previewed before its dimensions were ever cached waits a beat for mpv's live size. `[space]` again or `[esc]` closes it, and while it is open no other key reaches the file list behind it. Audio playlists have no preview (audio plays inline in Manager); zero or multiple selection does nothing.
