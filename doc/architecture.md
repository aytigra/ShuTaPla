...
A single-instance `Window(id:)` scene whose content switches between `WelcomeView` / `ManagerView` / `PlayerView` on `appState.mode`, plus a `Settings` scene. The scene type is what makes the app single-window, and the rest of the design depends on it: `AppState` holds window-level state (mode, managed playlist, selection, filters, the pending confirmation, the window frame) in one app-scoped object, every modal is presented from one app-wide host, and `HotkeyRouter` owns bare keys through an app-wide monitor — all of which assume exactly one `RootView`. The "New Window" command is removed as well (`CommandGroup(replacing: .newItem) {}`), but that removes a menu item, not the capability.

Player mode **claims** the window's fullscreen state through `WindowFullscreenBridge`, mounted once in `RootView` and driven by the mode: entering Player takes the window fullscreen, leaving it restores what the window was when the claim was taken, and a fullscreen the user chooses themselves stands — in Player mode or out of it. `FullscreenController` (`Extensions/NSWindow+Fullscreen.swift`) owns the transition: a state that has been asked for is owed until the window is observed in it, because AppKit drops a `toggleFullScreen` issued mid-animation and one issued at a hidden window starts a transition that never finishes.

Closing the window hides it but keeps the app running. The window and the whole view tree stay alive, so window-scoped state survives a close and a Dock reopen restores prior state — including the fullscreen the close took away, which AppKit does not put back on its own.

### Manager shell

The shell is an AppKit `NSSplitViewController` (`ManagerSplitScene`) hosting three SwiftUI panes (`PlaylistSidebar`, `PlaylistCenterView`, `TagSidebar`) in `NSHostingController`s, bridged into the SwiftUI scene via `NSViewControllerRepresentable`. AppKit is used because SwiftUI can't give a custom `NSToolbar` with regions aligned over the split panes: `NSTrackingSeparatorToolbarItem`s pinned to the dividers bound a center region (managed playlist name + type actions) and a trailing region (tag controls). The playlist name is the one toolbar item carrying no hosted view: `NSToolbarItem` does not re-measure a custom view whose content changes width, so a hosted label holds the *previous* playlist's width until something unrelated forces a toolbar layout pass — a longer name renders truncated to a few letters until then. A title-only item is sized by the toolbar itself, and `ToolbarTitleBinding` writes the name onto it; its leading inset is an en space in the string, since a spacer item costs ~20pt of inter-item spacing even at zero width. The sidebar's own controls — the scope tabs and New Playlist `+` — are not toolbar items at all, because a toolbar with a region too narrow for its items *must* push them into the overflow menu and offers no way to opt out, and that region is narrowest exactly when the sidebar is. They live instead in a leading `NSTitlebarAccessoryViewController` (`SidebarToolbarAccessory`) spanning from the traffic lights to the sidebar divider, which also derives the sidebar pane's minimum thickness from the two controls' measured widths so a drag can never narrow the pane past them. A collapsed sidebar leaves the accessory nothing to span, so it steps out of the titlebar and the two controls become toolbar items for the duration, placed *after* the sidebar tracking separator — the center region, which a collapsed sidebar leaves the full window width — where the system draws them the same Liquid Glass group capsules as every other toolbar group. Their placement math is a pure, unit-tested `SidebarToolbarLayout`. `ManagerChrome` (an `@Observable`: `sidebarCollapsed`, `inspectorVisible`, `managingTags`) is the shared source of truth both controller and panes read; the two pane flags persist to `UserDefaults` beside the divider positions the split view autosaves, since the chrome is `@State` on a view Player mode takes off the tree and would otherwise reopen at its defaults on every mode switch. Scope tabs are a custom toggle (`ScopeTabButton`) rather than a segmented `Picker` so a click on the already-active scope can collapse the sidebar. Sidebar rows come from a plain fetch (`AppState.playlists(ofType:)`) re-run when the observed `AppState.playlistsVersion` moves — the app mounts no live `@Query`, because a query refaults every registered instance of its entity on any store save, so an unrelated per-file write re-rendered the whole Manager. Only the mutations that change a visible row (playlist create/delete/rename/reorder, and the file create/delete paths behind `fileCount`) bump the version. A bump wakes the list body but not its rows — the body rebuilds an identical `ForEach` value from the same `Playlist` references, so SwiftUI keeps the rows it has. Anything derived from an untracked fetch therefore reads the gate in the view that displays it: the row's file count lives in `PlaylistRowBadge`, a leaf reading `AppState.fileCount(of:)`, shared with the `LibrarySurface` selector.
...
`PagedList` is a `ScrollView` over a `VStack` of fixed-height **pages** (a fixed row count each): every page reserves its exact height whether or not its rows are built, so the whole content lays out at honest size and the open-jump sets the scroll offset **imperatively** — one-way, never read back in `body` — landing on the right row. (A `LazyVStack` here would *estimate* the un-built pages above a deep target instead of honoring their frames, landing the jump several rows off.) Each page then windows itself: it measures its own frame against the viewport and builds its rows only while within a viewport of the screen, so scrolling re-renders only the pages crossing that band, never the container. Until the container has positioned, pages stay empty placeholders, so no page above the target builds a cell before the jump lands. A playlist switch remounts the tree (`.id(playlist)`) to re-open from cold through the same positioned path rather than animating off a stale offset; `GalleryPagedList` wraps the same core with a grid layer. The one-shot windowing math (content height, target/reveal offsets, item↔row chunking) is pure and unit-tested.
