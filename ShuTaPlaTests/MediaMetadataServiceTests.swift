...
@Suite struct MediaMetadataServiceTests {
    ...
    @MainActor @Test func completenessGuardIsTypeAware() throws {
        ...
        // Video needs duration + dimensions; audio only duration; image only dimensions. HDR gates
        // the *thumbnail*, not this metadata bundle, so no type requires it here.
        file.duration = 5
        ...
        file.width = 320
        file.height = 240
        #expect(file.hasCompleteMetadata(for: .video))    // duration + dimensions complete it
        #expect(file.hasCompleteMetadata(for: .image))    // image needs only dimensions
